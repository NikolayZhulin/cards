...
import { formatDate } from './PacksListSearch'
...
export const FullPackSearch = () => {
  const param = useParams()
  let params: FetchCardsRequestType = {
    cardAnswer: '',
    cardQuestion: '',
  ...
  const fetch = (params: FetchCardsRequestType) => {
    trigger(params).then(({ data }) => {
      if (data) {
        const { cards } = data
  ...
  useEffect(() => {
    fetch({ ...params, cardQuestion: debouncedValue })
  }, [debouncedValue])
  ...
  return (
    <TablePageStyle>
      ...
      <MiddleSection>
        <WideSearchBlock>
          <Title>Search</Title>
          <Input value={searchInputValue} onChange={changeSearchInputValue} />
        </WideSearchBlock>
      </MiddleSection>
